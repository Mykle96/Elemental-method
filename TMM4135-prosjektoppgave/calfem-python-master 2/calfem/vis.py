...
global visApp
visApp = vv.functions.use('qt5') # use qt4
...
def _preMeshDrawPrep(axes, coords, edof, dofsPerNode, elType):
    ...
    if elType in [1]:      #two node elements
        verticesPerFace = 2 #draw as a zero area rectangle
    elif elType in [2, 4, 9]: #elements with triangular faces
        verticesPerFace = 3
    elif elType in [3,5,10,16]: #elements with rectangular faces
        verticesPerFace = 4
    ...
    elif elType == 9: #if 6-node triangle
        faces = faces[:, 0:3] #The first 3 nodes are the corners of the traingle.
    elif elType == 10: #if 9-node-quads:
